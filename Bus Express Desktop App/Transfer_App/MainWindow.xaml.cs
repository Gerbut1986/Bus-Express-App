namespace Transfer_App
{
    ...
    public partial class MainWindow : Window
    {
        ...
        public static IEnumerable<Destination> Destinations { get; set; }
        #endregion
        ...
        {
            Title = _Title;
            selDestination = DestinationGrid.SelectedItem as Destination;
        ...
        #region Destinations Button's event:
        private void AddDest_Btn_Click(object sender, RoutedEventArgs e)
        {
            var select = (sender as Button).Content;
            var add = new AddDestinationWnd("Add", db);
            if (add.Title == "Exists")
            {
                Title = $"{select} пункт призначення існує!";
                return;
            }
            selectedPlace = select.ToString();
            add.Owner = this;
        ...
        private void DeleteDest_Btn_Click(object sender, RoutedEventArgs e)
        {
            if (notSelected)
            {
            ...
            {
                Title = new ServiceDestinations().Delete(selDestination.Id);
                RefreshGrid(true);
            }
        }

        private void EditDest_Btn_Click(object sender, RoutedEventArgs e)
        {
            if (notSelected)
            {
                Title = "Спочатку виберіть запис зі списку!";
            ...
            {
                var editForm = new AddDestinationWnd("Edit", selDestination, db);
                editForm.Owner = this;
                editForm.Show();
            }
        }

        private void ExportDest_Btn_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (sfd.ShowDialog() == true)
            {
                ...
                var dataToExport = Destinations.ToList();
                ExcelExporter<Destination>.ExportDataToExcel(dataToExport, path);
            }
        }
        #endregion

        #region Passengers Button's events:
        ...
        private async void ExportPass_Btn_Click(object sender, RoutedEventArgs e)
        {
            var sfd = new SaveFileDialog();
            ...
            if (sfd.ShowDialog() == true)
            {
                ...
                await Task.Run(() => { ExcelExporter<PassInfo>.ExportDataToExcel(dataToExport, path); });
            }
        }
        ...
        public void RefreshGrid(bool isAction)
        {
            ...
            Destinations = isAction ? new DataContext().Destinations.ToList() : db.Destinations.ToList();
            DestinationGrid.ItemsSource = Destinations;
        ...
        private void FilterDestDate() // No Usage..
        {
